...
![A flow representing the Semantic Kernel's process. An Ask is fed into the Kernel, which then uses the Planner to conduct a number of steps marked S, M, and C. These, then lead to a Get request which is passed off to an LLM API.](sk-flow.png)
