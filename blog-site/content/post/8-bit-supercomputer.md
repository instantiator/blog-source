...
Everything was going swimmingly (although on some days I ran pretty close to the memory limits of the BBC) until day 11. I completed the first part of the problem in reasonable time, but the second one had me stumped. There were 3 difficult aspects to this particular puzzle...
...
The poor little machine was not built to do this kind of work!

## 1. The remainder theorem thing

Aspect 1 caught me out at first, but once I'd grasped this, I was able to make progress:
...
At first, the solution I had put together seemed to be working, but then disaster struck! Despite reducing each value by the product of the numbers found in the real problem (`2 x 3 x 5 x 7 x 11 x 13 x 17 x 19 = 9699690`), the values reached during the calculations were still far too large and the program crashed out...
...
David Tall's article, on the other hand, was very promising! It describes how to use some procedures and functions he developed, that work on arbitrary numbers stored as strings. The code itself was almost certainly provided on a tape or disk that came with the magazine - but sadly that was not available.
...
So I now had functions that can parse numbers of indefinite size! (Well, limited by strings in BBC BASIC - which gives an upper limit of `255` characters. Still, that's much bigger than before, and easily big enough...)

_A trick I picked up was to initialise strings to their maximum length when they are first defined. If a string variable is replaced with a string that's longer, the original becomes wasted space and the computer must claim additional memory (from a very limited supply!) By initialising each string at maximum length, the space can be reused._

## 3. 10,000 iterations

Everything was back on track - I could do my calculations without crashing the machine, but there was another problem. This solution going to take a very long time to run! [BBC BASIC is a fast implementation of BASIC](https://stardot.org.uk/forums/viewtopic.php?t=19819) (for which a lot of credit is attributed to Sophie Wilson's work) - but it's all running on a 6502 processor (clocked at ~2Mhz), and it's an interpreted language.

Add to that, the laboriously slow big number arithemtic library I'd just written, and multiply the work to 10,000 iterations, and we have a problem!
...
This reduced my runtime to 13 minutes. It's still quite long - but good enough for a few iterations to get the solution just right.
...
I was particularly interested in fast terminal mode that it offers, so that's all I've scripted. With docker installed on your system, you can:
...
NB. It seems like I/O is pretty slow for beebjit in this mode, but when it's not printing to the screen it's _very_ fast!
